...
L.GeoCSV = L.GeoJSON.extend({
  ...
  addData: function (data) {
    if (typeof data === 'string') {
      //read titles
      var mytitles = this.options.titles;
      if (this.options.firstLineTitles) {
        ...
        mytitles = mytitles.trim().split(this.options.fieldSeparator);
        for (var i=0; i<mytitles.length; i++) {
          mytitles[i] = this._deleteDoubleQuotes(mytitles[i]);
        }
        this.options.titles = mytitles;
      }
      //generate _propertiesNames
      for (var i=0; i<mytitles.length; i++) {
         var prop = mytitles[i].toLowerCase().replace(/[^\w ]+/g,'').replace(/ +/g,'_');
  ...
  _csv2json: function (csv) {
    ...
    for (var num_linea = 0; num_linea < csv.length; num_linea++) {
      var campos = csv[num_linea].trim().split(this.options.fieldSeparator)
        , lng = parseFloat(campos[mytitles.indexOf(this.options.longitudeTitle)])
        , lat = parseFloat(campos[mytitles.indexOf(this.options.latitudeTitle)]);
      if (campos.length==mytitles.length && lng<180 && lng>-180 && lat<90 && lat>-90) {
        var feature = {};
        ...
        //properties
        for (var i=0; i<mytitles.length; i++) {
          if (mytitles[i] != this.options.latitudeTitle && mytitles[i] != this.options.longitudeTitle) {
